...
/**
 * Vector manipulation tools. READY
 *
...
public class VectorTools {
    ...
    /**
     * Matricies are x, by y Matrix.length = height, Matrix[0].length = width
     *
     * @param a
     * @param b
     * @return
    ...
    public static double[][] dists(double[][] a) {
        ...
        for (int x = 0; x < ret.length; x++) {
            for (int y = x + 1; y < ret.length; y++) {
                ret[x][y] = ret[y][x] = VectorTools.dot(
                        VectorTools.normalize(a[x]),
                        VectorTools.normalize(a[y]));
            }
        }
        return ret;
    }
